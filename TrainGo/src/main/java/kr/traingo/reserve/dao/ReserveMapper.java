...
@Update("update seats set booker=#{id} where trainnum=#{trainnum} and seatnum=#{seatnum}")
public void registerSeat(SeatSelectedCommand command);
                                                                                                    //이 부분 ""으로 바꾸는 거 그냥 없는 값도 받도록 만들자.
@Insert("insert into ticketlist (ticketnum,id,trainnum,seatnum,cancel,discount,cost) values (ticketlist_seq.nextval,#{id},#{trainnum},#{seatnum},'cancelP',#{discount},#{cost})")
...
@Insert("merge into autotrain a using (select * from dual) b on (a.trainname like #{trainGradeName} and a.departtime=to_date(#{depPlandTime},'yyyy-mm-dd hh24:mi')) when not matched then insert (a.trainnum,a.trainname,a.seats,a.departsta,a.departtime,a.arrivalsta,a.arrivaltime,a.charge) values (autotrain_seq.nextval,#{trainGradeName},240,#{depPlaceName},to_date(#{depPlandTime},'yyyy-mm-dd hh24:mi'),#{arrPlaceName},to_date(#{arrPlandTime},'yyyy-mm-dd hh24:mi'), #{adultCharge})")
public void insertAutoTrain(ResultTimeTableCommand resultTimeTableCommand);
