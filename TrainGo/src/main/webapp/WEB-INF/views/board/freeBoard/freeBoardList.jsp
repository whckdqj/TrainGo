...
<div class="page-main-style">
	<form action="freelist.do" cssClass="form-horizontal" enctype="multipart/form-data" id="write_form" method="get">
	<legend>Free Board</legend>
		<h4>Free Board List</h4>
		...
		<c:if test="${count>0}">
			<table class="table table-striped">
				...
				<c:forEach var="article" items="${freeList}">
					<tr>
						<td>${article.num}</td>
						<td><a href="freedetail.do?num=${article.num}">${article.free_title}</a></td>
         ...
         <div class="col-xs-10 col-xs-offset-2">
                <div class="btn-group btn-group-justified" role="group">
                    <div class="btn-group" role="group">
                        ...
                        <button type="button" class="btn btn-danger" onclick="location.href='${pageContext.request.contextPath}/board/freeBoard/freewrite.do'">글쓰기</button>
                        </c:if>
                        <c:if test="${empty userId}">
                        <button type="button" class="btn btn-danger" disabled="disabled" onclick="location.href='${pageContext.request.contextPath}/board/freeBoard/freewrite.do'">글쓰기</button>
                        </c:if>
                    </div>
                    <div class="btn-group" role="group">
